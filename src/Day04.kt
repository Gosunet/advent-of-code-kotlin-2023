...
fun main() {
  fun parse(input: List<String>) = input.withIndex().map { (index, line) ->
    val cardNumbers = line.split(":")[1].replace("  ", " ").split(" | ")
    val playedNumbers = cardNumbers[0].trimStart().split(" ").map { it.toInt() }
    val winningsNumbers = cardNumbers[1].trimStart().split(" ").map { it.toInt() }
    Card(index, playedNumbers, winningsNumbers)
  }

  fun part1(input: List<String>): Int =
    parse(input).map(Card::points).sum()
